<nav x-data="{ open: false }" class="fixed top-0 w-full bg-white border-b border-gray-200 shadow">
        ...
        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
          ...
          @foreach (Auth::user()->allTeams() as $team)
            ...
            <form method="POST" action="{{ route('current-team.update') }}">
              ...
              <x-jet-nav-link href="#" onclick="event.preventDefault(); this.closest('form').submit();"
                :active="Auth::user()->isCurrentTeam($team)" class="flex items-center h-full">
                {{ $team->name }}
              </x-jet-nav-link>
            </form>
          @endforeach
